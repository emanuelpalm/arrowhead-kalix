...
public class DtoSpecificationEncodingXml implements DtoSpecificationEncoding {
    @Override
    public DtoEncoding encoding() {
        return DtoEncoding.XML;
    }

    @Override
    public void implementFor(final DtoTarget target, final TypeSpec.Builder implementation) {
        if (target.interfaceType().isReadable(DtoEncoding.XML)) {
            implementation.addSuperinterface(XmlReadable.class);
            //implementReadMethodsFor(target, implementation);
        }
        if (target.interfaceType().isWritable(DtoEncoding.XML)) {
            implementation.addSuperinterface(XmlWritable.class);
...
/*
    private void implementReadMethodsFor(final DtoTarget target, final TypeSpec.Builder implementation) throws DtoException {
        ...
        implementation.addMethod(MethodSpec.methodBuilder("readXml")
            .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
            .returns(dataTypeName)
            .addParameter(BinaryReader.class, "source", Modifier.FINAL)
            .addException(DtoReadException.class)
            .addStatement("return readXml($T.tokenize(source))", XmlTokenizer.class)
            .build());

        final var builder = MethodSpec.methodBuilder("readXml")
            .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
            .returns(dataTypeName)
            .addParameter(XmlTokenBuffer.class, "buffer", Modifier.FINAL)
            .addException(DtoReadException.class)
            .addAnnotation(Internal.class)
            .addStatement("final var source = buffer.source()")
            .addStatement("var token = buffer.next()")
            .addStatement("var type = ($T) null", XmlTokenType.class)
        ...
        builder
            .beginControlFlow("if (token.type() != $T.ELEMENT)", XmlTokenType.class)
            .addStatement("errorMessage = \"Expected element\"")
        ...
        for (final var property : properties) {
            try {
                builder.beginControlFlow("case $S:", property.nameFor(DtoEncoding.XML));
        ...
        builder
            ...
            .addStatement("throw new $1T($2T.XML, errorMessage, atEnd " +
    ...
    private void readArray(final DtoSequence type, final Expander assignment, final MethodSpec.Builder builder) {
        ...
        if (level == 0) {
            builder
                .beginControlFlow("if (type == $T.NULL)", XmlTokenType.class)
                .addStatement("continue")
                .endControlFlow();
        }
        builder
            .beginControlFlow("if (type != $T.ARRAY)", XmlTokenType.class)
    ...
    private void readCustom(final DtoElement type, final Expander assignment, final MethodSpec.Builder builder) {
        if (level == 0) {
            builder
                .beginControlFlow("if (buffer.peek().type() == $T.NULL)", XmlTokenType.class)
                .addStatement("buffer.skipElement()")
                .addStatement("continue")
                .endControlFlow();
        }

        builder.addStatement(assignment.expand("$T.readXml(buffer)"), type.inputTypeName());
    }
    ...
    private void readEnum(final DtoType type, final Expander assignment, final MethodSpec.Builder builder) {
        ...
        builder
            .beginControlFlow("if (type != $T.STRING)", XmlTokenType.class)
    ...
    private void readInterface(final DtoInterface type, final Expander assignment, final MethodSpec.Builder builder) {
        if (!type.isReadable(DtoEncoding.XML)) {
            throw new IllegalStateException(type.simpleName() + " is not annotated with " +
                "@DtoReadableAs, or lacks DataEncoding.XML as annotation argument");
        }
    ...
    private void readMap(final DtoMap type, final Expander assignment, final MethodSpec.Builder builder) {
        ...
        builder
            .beginControlFlow("if (type != $T.OBJECT)", XmlTokenType.class)
    ...
    private void readNumber(final String type, final Expander assignment, final MethodSpec.Builder builder) {
        ...
        builder
            .beginControlFlow("if (type != $T.NUMBER)", XmlTokenType.class)
            .addStatement("errorMessage = \"Expected number\"")
            .addStatement("break error")
            .endControlFlow()
    ...
    private void implementWriteMethodFor(final DtoTarget target, final TypeSpec.Builder implementation)
        throws DtoException
    {
        final var builder = MethodSpec.methodBuilder("writeXml")
        ...
        for (var p0 = 0; p0 < p1; ++p0) {
            ...
            try {
                ...
                writeCache
                    .append('"')
                    .append(property.nameFor(DtoEncoding.XML))
                    .append("\":");
    ...
    private void writeOther(final String name, final MethodSpec.Builder builder) {
        writeCache.addWriteIfNotEmpty(builder);
        builder.addStatement("$T.write($N, target)", XmlWrite.class, name);
    }
    ...
    private static IllegalStateException characterTypesNotSupportedException() {
        return new IllegalStateException("The char and Character types " +
            "cannot be represented as XML; either change the type or " +
            "remove DataEncoding.XML from the array of encodings " +
